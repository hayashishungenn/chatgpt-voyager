...
ChatGPT Voyager の「引用返信」機能を使うと、文脈を保った返信をより正確かつ効率的に行えます。
...
1. **選択して引用**：会話内（あなたの質問でも ChatGPT の回答でも）で、任意のテキストを選択します。
...
- **標準フォーマット**：一般的な Markdown 構文を使うため、ChatGPT が引用として正しく解釈しやすくなり、より的確な応答を引き出せます。
- **複数行対応**：複数行のテキストを選択した場合、Voyager は各行に引用記号を自動的に追加し、フォーマットを整えます。

## 活用テクニック

- **詳細を掘り下げる**：ChatGPT の回答の中で不明確な概念を選択して引用し、「この概念について詳しく説明してください」と入力します。
