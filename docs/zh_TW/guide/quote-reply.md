...
ChatGPT Voyager 提供了便捷的「引用回覆」功能，讓針對特定內容的回覆更加精準高效。
...
1. **選中即引**：在對話頁面（無論是你的提問還是 ChatGPT 的回答）中，用滑鼠選中任意一段文字。
...
- **標準格式**：使用通用的 Markdown 語法，ChatGPT 可以完美理解這種引用結構，從而做出更精準的回應。
- **多行支持**：如果選中了多行文本，Voyager 會自動為每一行添加引用符號，保持格式整潔。

## 使用技巧

- **追問細節**：選中 ChatGPT 回答中不清楚的某個概念，點擊引用，然後輸入「請詳細解釋一下這個概念」。
