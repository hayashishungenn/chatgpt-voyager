...
export class MarkdownFormatter {
  ...
  private static formatHeader(metadata: ConversationMetadata): string {
    ...
    lines.push(`**Source**: [ChatGPT Chat](${metadata.url})`);
  ...
  private static formatContent(content: string): string {
    ...
    // Detect and preserve code blocks (already formatted by ChatGPT)
  ...
  private static formatFooter(metadata: ConversationMetadata): string {
    return [
      `*Exported from [ChatGPT Voyager](https://github.com/hayashishungenn/chatgpt-voyager)*`,
  ...
  static generateFilename(): string {
    ...
    return `chatgpt-chat-${y}${m}${day}-${hh}${mm}${ss}.md`;
  }
