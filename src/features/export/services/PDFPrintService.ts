...
export class PDFPrintService {
  ...
  private static getConversationTitle(): string {
    ...
    try {
      ...
      console.debug(
        '[PDF Export] Failed to get title from header:',
        error,
      );
    }
  ...
  private static renderFooter(metadata: ConversationMetadata): string {
    return `
      <footer class="gv-print-footer">
        <p>Exported from <a href="https://github.com/hayashishungenn/chatgpt-voyager">ChatGPT Voyager</a> • ${metadata.count} conversation turns</p>
