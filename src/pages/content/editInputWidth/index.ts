...
const STYLE_ID = 'chatgpt-voyager-edit-input-width';
...
export function startEditInputWidthAdjuster(): void {
  ...
  chrome.storage?.sync?.get({ chatgptEditInputWidth: DEFAULT_PERCENT }, (res) => {
    const storedWidth = res?.chatgptEditInputWidth;
    ...
    if (typeof storedWidth === 'number' && storedWidth !== normalized) {
      try {
        ...
        console.warn('[ChatGPT Voyager] Failed to migrate edit input width to %:', e);
      }
    }
  });
  ...
  chrome.storage?.onChanged?.addListener((changes, area) => {
    if (area === 'sync' && changes.chatgptEditInputWidth) {
      const newWidth = changes.chatgptEditInputWidth.newValue;
      if (typeof newWidth === 'number') {
        ...
        if (normalized !== newWidth) {
          try {
            ...
            console.warn('[ChatGPT Voyager] Failed to migrate edit input width to % on change:', e);
          }
        }
      }
    }
