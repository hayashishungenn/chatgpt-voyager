...
export async function startPromptManager(): Promise<{ destroy: () => void }> {
  ...
  try {
    ...
    titleText.textContent = 'ChatGPT Voyager';
    ...
    const releaseUrl = manifestVersion
      ? `https://github.com/hayashishungenn/chatgpt-voyager/releases/tag/${currentReleaseTag ?? `v${manifestVersion}`}`
    ...
    (async () => {
      ...
      const latestUrl = latestTag
        ? `https://github.com/hayashishungenn/chatgpt-voyager/releases/tag/${latestTag}`
    ...
    gh.href = 'https://github.com/hayashishungenn/chatgpt-voyager';
    gh.target = '_blank';
    gh.rel = 'noreferrer';
    ...
    exportBtn.addEventListener('click', async () => {
      try {
        ...
        const payload = {
          format: 'chatgpt-voyager.prompts.v1',
    ...
    importInput.addEventListener('change', async () => {
      ...
      try {
        ...
        if (!json || (json.format !== 'chatgpt-voyager.prompts.v1' && !Array.isArray(json.items))) {
