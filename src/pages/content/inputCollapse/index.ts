...
const STYLE_ID = 'chatgpt-voyager-input-collapse';
 ...
 * ChatGPT conversation pages include /c/<id> in the pathname.
 */
function isHomepageOrNewConversation(): boolean {
  const pathname = window.location.pathname;
  return !/\/c\/[a-zA-Z0-9_-]+/.test(pathname);
}

/**
 * Checks if the current page is a GPT builder/editor page.
 * These pages should not have auto-collapse behavior.
 */
function isGemsEditorPage(): boolean {
  const pathname = window.location.pathname;
  return /^\/gpts\/(editor|new)\/?/.test(pathname) || /^\/gpt-builder\/?/.test(pathname);
}
...
function getInputContainer(): HTMLElement | null {
  const textarea = document.querySelector(
    'textarea#prompt-textarea, textarea[data-testid="prompt-textarea"], textarea[name="prompt"], rich-textarea',
  );
  ...
  for (let i = 0; i < 8; i++) {
    ...
    // Check for specific container classes or roles
...
function isInputEmpty(container: HTMLElement): boolean {
  ...
  const attachmentsArea =
    ...
    container.querySelector('[data-testid="file-preview"]');
...
export function startInputCollapse() {
  ...
  chrome.storage?.sync?.get({ gvInputCollapseEnabled: false }, (res) => {
    if (res?.gvInputCollapseEnabled === false) {
      ...
      console.log('[ChatGPT Voyager] Input collapse is disabled');
      return;
    }
...
function expand(container: HTMLElement) {
  if (container.classList.contains(COLLAPSED_CLASS)) {
    ...
    const editor =
      ...
      container.querySelector('textarea#prompt-textarea, textarea[data-testid="prompt-textarea"]') ||
