...
const STYLE_ID = 'chatgpt-voyager-chat-width';
...
function applyWidth(widthPercent: number) {
  ...
  style.textContent = `
    ...
    /* ChatGPT layout containers */
    main > div:has(article[data-testid^="conversation-turn-"]),
...
export function startChatWidthAdjuster() {
  ...
  chrome.storage?.sync?.get({ chatgptChatWidth: DEFAULT_PERCENT }, (res) => {
    const storedWidth = res?.chatgptChatWidth;
    ...
    if (typeof storedWidth === 'number' && storedWidth !== normalized) {
      try {
        ...
        console.warn('[ChatGPT Voyager] Failed to migrate chat width to %:', e);
      }
    }
  });
  ...
  const storageChangeHandler = (changes: any, area: string) => {
    if (area === 'sync' && changes.chatgptChatWidth) {
      const newWidth = changes.chatgptChatWidth.newValue;
      if (typeof newWidth === 'number') {
        ...
        if (normalized !== newWidth) {
          try {
            ...
            console.warn('[ChatGPT Voyager] Failed to migrate chat width to % on change:', e);
          }
        }
      }
    }
