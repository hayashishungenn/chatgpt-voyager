...
export class AIStudioFolderManager {
  ...
  private handleExport(): void {
    ...
    downloadJSON(payload, `chatgpt-voyager-folders-${this.timestamp()}.json`);
  }
  ...
  private setupStorageListener(): void {
    browser.storage.onChanged.addListener((changes, areaName) => {
      if (areaName === 'sync' && changes.chatgptFolderEnabled) {
  ...
  private showNotification(message: string, level: 'info' | 'warning' | 'error' = 'error'): void {
    try {
      ...
      notification.textContent = `[ChatGPT Voyager] ${message} `;
