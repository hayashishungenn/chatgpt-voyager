...
const SIDEBAR_CONTAINER_SELECTOR =
  'nav[aria-label="Chat history"], nav[data-testid="chat-history"], [data-test-id="overflow-container"]';
const CONVERSATION_LIST_SELECTOR =
  '[data-test-id="all-conversations"], [data-testid="conversation-list"], [data-testid="history-list"], .chat-history';
const CONVERSATION_ITEM_SELECTOR =
  '[data-test-id="conversation"], [data-testid^="history-item"], a[href^="/c/"]';
...
export class FolderManager {
  ...
  constructor() {
    ...
    this.backupService = new DataBackupService<FolderData>('chatgpt-folders', validateFolderData);
  ...
  private findRecentSection(): void {
    ...
    let conversationsList = this.sidebarContainer.querySelector(CONVERSATION_LIST_SELECTOR);
    ...
    if (!conversationsList) {
      ...
      const conversationItems = this.sidebarContainer.querySelectorAll(CONVERSATION_ITEM_SELECTOR);
  ...
  private makeConversationsDraggable(): void {
    ...
    const conversations = this.sidebarContainer.querySelectorAll(CONVERSATION_ITEM_SELECTOR);
  ...
  private findConversationElement(conversationId: string): HTMLElement | null {
    ...
    const nativeConvs = this.sidebarContainer?.querySelectorAll(CONVERSATION_ITEM_SELECTOR);
    if (nativeConvs) {
  ...
  private extractConversationId(element: HTMLElement): string {
    ...
    // This ensures we can still identify conversations even if the UI changes
    const link = element.querySelector(
      'a[href*="/c/"], a[href*="/app/"], a[href*="/gem/"]',
    ) as HTMLAnchorElement | null;
    if (link) {
      const href = link.href;
      // Try /c/<id>
      let match = href.match(/\/c\/([^\/?#]+)/);
      if (match && match[1]) {
        return `c_${match[1]}`;
      }
      // Try /app/<hexId>
      match = href.match(/\/app\/([^\/?#]+)/);
      if (match && match[1]) {
  ...
  private extractConversationData(element: HTMLElement): {
    ...
    // Use /c/{id} for ChatGPT (fallback to legacy /app/{id})
    url += `/c/${hexId}`;
   ...
   * @returns The conversation ID (without 'c_' prefix) or undefined if not found
   *
   * @remarks
   * This method attempts two extraction strategies:
   * 1. From jslog attribute (e.g., jslog="c_abc123def456")
   * 2. From href in anchor tags (e.g., /c/uuid, /app/abc123def456 or /gem/xxx/abc123def456)
   */
  private extractConversationIdFromElement(element: Element): string | undefined {
    ...
    if (link) {
      ...
      return chatMatch?.[1] || appMatch?.[1] || gemMatch?.[1];
    }
    ...
    this.conversationObserver = new MutationObserver((mutations) => {
      ...
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          if (node instanceof HTMLElement) {
            ...
            if (node.matches(CONVERSATION_ITEM_SELECTOR)) {
            ...
            const conversations = node.querySelectorAll(CONVERSATION_ITEM_SELECTOR);
            conversations.forEach((conv) => {
      ...
      mutations.forEach((mutation) => {
        mutation.removedNodes.forEach((node) => {
          if (node instanceof HTMLElement) {
            const isConv = node.matches(CONVERSATION_ITEM_SELECTOR);
            // Check if it contains conversations (e.g. a container was removed)
            const containedConvsCount = node.querySelectorAll(CONVERSATION_ITEM_SELECTOR).length;
      ...
      nodesWithRemovals.forEach((node) => {
        const conversations = node.matches(CONVERSATION_ITEM_SELECTOR)
          ? [node]
          : Array.from(node.querySelectorAll(CONVERSATION_ITEM_SELECTOR));
  ...
  private updateVisibilityBasedOnSideNav(): void {
    ...
    // During screen resize (e.g., split-screen to fullscreen), the host may re-render the sidebar DOM,
  ...
  /**
   * Batch delete native conversations by simulating user clicks
   * This triggers the actual deletion on the host's servers
   */
  ...
  private findNativeConversationElement(conversationId: string): HTMLElement | null {
    ...
    const allConversations = this.sidebarContainer?.querySelectorAll(CONVERSATION_ITEM_SELECTOR);
  ...
  private async confirmDeleteIfNeeded(): Promise<void> {
    ...
    // The host may use a dialog with confirm/cancel buttons
    const maxWaitTime = this.BATCH_DELETE_CONFIG.MAX_BUTTON_WAIT_TIME;
    const checkInterval = this.BATCH_DELETE_CONFIG.BUTTON_CHECK_INTERVAL;
    let elapsed = 0;
  ...
  private updateMultiSelectModeUI(): void {
    ...
    if (actionsContainer && this.isMultiSelectMode) {
      ...
      if (this.multiSelectSource === 'folder') {
        ...
        // Delete button for native multi-select (deletes from host)
        const deleteBtn = document.createElement('button');
        deleteBtn.className = 'gv-multi-select-action-btn gv-multi-select-delete-btn';
        deleteBtn.innerHTML =
          '<mat-icon role="img" class="mat-icon notranslate google-symbols mat-ligature-font mat-icon-no-color" aria-hidden="true">delete</mat-icon>';
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        if (moreButton) {
          ...
          // Strategy 1: In some UIs, the conversation div and actions-menu-button are siblings
          ...
          if (actionsContainer) {
            ...
            // Look for previous sibling with conversation marker
            let sibling = actionsContainer.previousElementSibling;
            while (sibling) {
              if (sibling.matches(CONVERSATION_ITEM_SELECTOR)) {
          ...
          if (!conversationEl) {
            ...
            conversationEl = moreButton.closest(CONVERSATION_ITEM_SELECTOR) as HTMLElement | null;
          }
  ...
  private extractNativeConversationId(conversationEl: HTMLElement): string | null {
    // Support /c/<id> (ChatGPT) and legacy /app/<hexId> /gem/<gemId>/<hexId>
    const scope =
      (conversationEl.closest(CONVERSATION_ITEM_SELECTOR) as HTMLElement) || conversationEl;

    // Get all conversation links
    const links = scope.querySelectorAll('a[href*="/c/"], a[href*="/app/"], a[href*="/gem/"]');
    ...
    let match = href.match(/\/c\/([^\/?#]+)/);
    if (match && match[1]) {
      this.debug('extractId: extracted from /c/', match[1]);
      return match[1];
    }
    // Try /app/<hexId>
    match = href.match(/\/app\/([^\/?#]+)/);
  ...
  private extractNativeConversationTitle(conversationEl: HTMLElement): string | null {
    ...
    const titleEl = scope.querySelector(
      '.gds-label-l, .conversation-title-text, [data-test-id="conversation-title"], [data-testid="conversation-title"], [data-testid^="history-item"] span, h3',
    );
  ...
  private syncConversationTitleFromNative(conversationId: string): string | null {
    try {
      ...
      const conversations = document.querySelectorAll(CONVERSATION_ITEM_SELECTOR);
      const rawId = conversationId.replace(/^c_/, '');
      for (const convEl of Array.from(conversations)) {
        ...
        if (jslog && (jslog.includes(conversationId) || jslog.includes(rawId))) {
        ...
        if (link && (link.href.includes(conversationId) || link.href.includes(rawId))) {
          const currentTitle = this.extractNativeConversationTitle(convEl as HTMLElement);
          if (currentTitle) {
   ...
   * This prevents false positives when the host UI temporarily removes/re-adds elements
   */
  ...
  private isConversationInDOM(conversationId: string): boolean {
    ...
    try {
      // Check by href (ChatGPT)
      const byHref = this.sidebarContainer.querySelector(
        `a[href*="/c/${conversationId}"], a[href*="${conversationId}"]`,
      );
      if (byHref) {
      ...
      // Legacy Gemini fallback: jslog attribute
      ...
      // Legacy Gemini fallback: href inside conversation item
      const byLegacyHref = this.sidebarContainer.querySelector(
        `[data-test-id="conversation"] a[href*="${conversationId}"]`,
      );
      if (byLegacyHref) {
        this.debug(`Found conversation ${conversationId} in DOM by legacy href`);
        return true;
      }
  ...
  private getCurrentConversationId(): string | null {
    ...
    return chatMatch?.[1] || appMatch?.[1] || gemMatch?.[1] || null;
  }
  ...
  private buildConversationUrlFromId(hexId: string): string {
    try {
      ...
      this.debug('Failed to build conversation URL:', e);
    }
    return `https://chatgpt.com/c/${hexId}`;
  }
  ...
  private extractNativeConversationUrl(conversationEl: HTMLElement): string | null {
    const scope =
      (conversationEl.closest(CONVERSATION_ITEM_SELECTOR) as HTMLElement) || conversationEl;
    const link = scope.querySelector('a[href*="/c/"], a[href*="/app/"], a[href*="/gem/"]');
    if (!link) {
  ...
  private getCurrentHexIdFromLocation(): string | null {
    try {
      ...
      // Match /c/<id> (ChatGPT) or legacy /app/<hex> /gem/<gemId>/<hex>
      const m = path.match(/\/c\/([a-zA-Z0-9_-]+)/) || path.match(/\/(?:app|gem\/[^/]+)\/([a-f0-9]+)/i);
      return m ? m[1] : null;
    } catch (e) {
  ...
  private async loadFolderEnabledSetting(): Promise<void> {
    try {
      const result = await browser.storage.sync.get({ chatgptFolderEnabled: true });
      this.folderEnabled = result.chatgptFolderEnabled !== false;
  ...
  private async loadHideArchivedSetting(): Promise<void> {
    try {
      const result = await browser.storage.sync.get({
        chatgptFolderHideArchivedConversations: false,
      });
      this.hideArchivedConversations = !!result.chatgptFolderHideArchivedConversations;
  ...
  private setupStorageListener(): void {
    ...
    browser.storage.onChanged.addListener((changes, areaName) => {
      if (areaName === 'sync') {
        if (changes.chatgptFolderEnabled) {
          this.folderEnabled = changes.chatgptFolderEnabled.newValue !== false;
          this.debug('Folder enabled setting changed:', this.folderEnabled);
          // Apply the change to folder visibility
          this.applyFolderEnabledSetting();
        }
        if (changes.chatgptFolderHideArchivedConversations) {
          this.hideArchivedConversations =
            !!changes.chatgptFolderHideArchivedConversations.newValue;
      ...
      for (const conv of Array.from(conversations)) {
        ...
        if (jslog && (jslog.includes(hexId) || (rawId && jslog.includes(rawId)))) {
          ...
          setTimeout(() => {
            ...
            // Check if URL changed after navigation
        ...
        const href = link?.getAttribute('href') || '';
        if (rawId && href.includes(rawId)) {
          (conv as HTMLElement).click();
          this.debug('Navigated by clicking sidebar element (by href match)');
          setTimeout(() => this.highlightActiveConversationInFolders(), 0);
          setTimeout(() => {
            if (conversation) {
              const syncedTitle = this.syncConversationTitleFromNative(hexId);
  ...
  private installSidebarClickListener(): void {
    // Capture clicks in the native sidebar and update highlight after navigation happens
    ...
    this.sidebarClickListener = (e: Event) => {
      ...
      if (!target) return;
      const a = target.closest('a[href*="/c/"], a[href*="/app/"], a[href*="/gem/"]') as HTMLAnchorElement | null;
      if (a) {
        setTimeout(() => this.highlightActiveConversationInFolders(), 0);
      }
    };
