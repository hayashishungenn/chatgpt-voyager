/* Adjust ChatGPT sidebar width with CSS overrides (keeps Gemini fallbacks for legacy layouts) */
...
function buildStyle(widthValue: number): string {
  ...
  return `
    :root {
      --gv-chatgpt-sidebar-width: ${clampedWidth};
    }

    /* ChatGPT sidebar */
    nav[aria-label="Chat history"],
    nav[data-testid="chat-history"] {
      width: var(--gv-chatgpt-sidebar-width) !important;
      min-width: var(--gv-chatgpt-sidebar-width) !important;
      max-width: var(--gv-chatgpt-sidebar-width) !important;
      flex: 0 0 var(--gv-chatgpt-sidebar-width) !important;
    }

    body:has(nav[aria-label="Chat history"]) main,
    body:has(nav[data-testid="chat-history"]) main {
      max-width: calc(100% - var(--gv-chatgpt-sidebar-width)) !important;
    }
...
export function startSidebarWidthAdjuster(): void {
  ...
  const measureTopBarWidth = () => {
    try {
      ...
      console.warn('[ChatGPT Voyager] Failed to measure top bar width:', err);
    }
  };

  // 1) Read initial width
  try {
    chrome.storage?.sync?.get({ chatgptSidebarWidth: DEFAULT_PX }, (res) => {
      const w = Number(res?.chatgptSidebarWidth);
      ...
      if (Number.isFinite(w) && w !== normalized) {
        try {
          chrome.storage?.sync?.set({ chatgptSidebarWidth: normalized });
        } catch (err) {
          console.warn('[ChatGPT Voyager] Failed to migrate sidebar width to %:', err);
        }
      }
    });
  } catch (e) {
    // Fallback: inject default value if no storage permission
    console.error('[ChatGPT Voyager] Failed to get sidebar width from storage:', e);
    applyWidth(currentWidthValue);
  }
  ...
  try {
    chrome.storage?.onChanged?.addListener((changes, area) => {
      if (area === 'sync' && changes.chatgptSidebarWidth) {
        const w = Number(changes.chatgptSidebarWidth.newValue);
        if (Number.isFinite(w)) {
          ...
          if (normalized !== w) {
            try {
              ...
              console.warn('[ChatGPT Voyager] Failed to migrate sidebar width to % on change:', err);
            }
          }
        }
      }
    ...
    console.error('[ChatGPT Voyager] Failed to add storage listener for sidebar width:', e);
  }
  ...
  try {
    ...
    console.warn('[ChatGPT Voyager] Failed to observe top bar width:', err);
  }
