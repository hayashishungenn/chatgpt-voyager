 ...
 *
 * Automatically detects and removes watermarks from generated images on the page.
 *
...
/**
 * Check if an image element is a valid generated image
 */
const isValidGeneratedImage = (img: HTMLImageElement): boolean =>
  img.closest('generated-image,.generated-image-container') !== null;

/**
 * Find all generated images on the page
 */
const findGeneratedImages = (): HTMLImageElement[] =>
  [...document.querySelectorAll<HTMLImageElement>('img[src*="googleusercontent.com"]')].filter(
    (img) => isValidGeneratedImage(img) && img.dataset.watermarkProcessed !== 'true',
  );
...
function addDownloadIndicator(imgElement: HTMLImageElement): void {
  ...
  // Try to find native download button area
...
async function processImage(imgElement: HTMLImageElement): Promise<void> {
  ...
  try {
    ...
    console.log('[ChatGPT Voyager] Watermark removed from preview image');

    // Add indicator to download button
    addDownloadIndicator(imgElement);
  } catch (error) {
    console.warn('[ChatGPT Voyager] Failed to process image for watermark removal:', error);
...
/**
 * Process all generated images on the page
 */
...
const setupMutationObserver = (): void => {
  ...
  console.log('[ChatGPT Voyager] Watermark remover MutationObserver active');
};
/**
  ...
  const observer = new MutationObserver(async () => {
    ...
    if (requestData) {
      ...
      try {
        ...
        console.error('[ChatGPT Voyager] Failed to parse request:', e);
      }
    }
  });

  observer.observe(bridge, { attributes: true, attributeFilter: ['data-request'] });
  console.log('[ChatGPT Voyager] Fetch interceptor bridge ready');
}
...
async function processImageRequest(
  ...
  try {
    ...
    console.error('[ChatGPT Voyager] Failed to process image:', error);
...
export async function startWatermarkRemover(): Promise<void> {
  try {
    ...
    const result = await chrome.storage?.sync?.get({ chatgptWatermarkRemoverEnabled: true });
    const isEnabled = result?.chatgptWatermarkRemoverEnabled !== false;

    // Notify MAIN world fetch interceptor about state
    notifyFetchInterceptor(isEnabled);

    if (!isEnabled) {
      console.log('[ChatGPT Voyager] Watermark remover is disabled');
      return;
    }
    ...
    console.log('[ChatGPT Voyager] Initializing watermark remover...');
    ...
    console.log('[ChatGPT Voyager] Watermark remover ready');
  } catch (error) {
    console.error('[ChatGPT Voyager] Watermark remover initialization failed:', error);
  }
}
...
function setupStatusListener(): void {
  ...
  const handleStatus = (statusData: string): void => {
    console.log('[ChatGPT Voyager] Status data received:', statusData);
    ...
    try {
      ...
      console.error('[ChatGPT Voyager] Failed to parse status:', e);
    }
  };
