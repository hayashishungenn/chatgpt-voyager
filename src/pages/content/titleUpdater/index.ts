 ...
 * Description: Automatically updates the browser tab title to match the current ChatGPT chat title.
...
function tryUpdateTitle() {
  ...
  if (currentTitle !== lastTitle) {
    document.title = `${currentTitle} - ChatGPT`;
...
function findChatTitle(): string | null {
  // Only run on conversation pages: /c/<id>
  ...
  if (titleEl) {
    ...
    if (text && text !== 'New chat' && text !== 'New Chat' && text !== 'ChatGPT') {
      return text;
    }
  }

  const sidebarActive = document.querySelector('nav a[aria-current="page"]');
  if (sidebarActive) {
