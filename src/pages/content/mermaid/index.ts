...
const renderMermaid = async (codeBlock: HTMLElement, code: string) => {
  ...
  try {
    ...
    codeBlock.dataset.mermaidProcessing = 'false';
    console.log('[ChatGPT Voyager] Mermaid diagram rendered:', uniqueId);
  } catch (error) {
    codeBlock.dataset.mermaidProcessing = 'false';
...
const getCodeBlockLanguage = (codeEl: Element): string | null => {
  ...
  // Some hosts use: <div class="code-block-decoration"><span>Language</span>...</div>
...
const processCodeBlocks = () => {
  ...
  codeElements.forEach((codeEl) => {
    ...
    // Check the language label from the code block header
...
export const startMermaid = () => {
  ...
  chrome.storage?.sync?.get({ gvMermaidEnabled: true }, (result) => {
    ...
    if (mermaidEnabled) {
      ...
      console.log('[ChatGPT Voyager] Mermaid rendering is disabled');
    }
  });
  ...
  chrome.storage?.onChanged?.addListener((changes, areaName) => {
    if (areaName === 'sync' && changes.gvMermaidEnabled) {
      ...
      if (mermaidEnabled) {
        initializeMermaid();
        console.log('[ChatGPT Voyager] Mermaid rendering enabled');
      } else {
        ...
        console.log('[ChatGPT Voyager] Mermaid rendering disabled');
      }
    }
  });
};
...
const initializeMermaid = () => {
  ...
  console.log('[ChatGPT Voyager] Mermaid integration started');
};
