...
export class TimelineManager {
  ...
  async init(): Promise<void> {
    ...
    // Ensure initial render even when ChatGPT DOM is already stable (no mutations after observer attaches)
    ...
    try {
      ...
      const defaults = {
        chatgptTimelineScrollMode: 'flow',
        chatgptTimelineHideContainer: false,
        chatgptTimelineDraggable: false,
        chatgptTimelineMarkerLevel: false,
        chatgptTimelinePosition: null,
      };
      ...
      if (g.chrome?.storage?.sync || g.browser?.storage?.sync) {
        ...
        const saved = localStorage.getItem('chatgptTimelineScrollMode');
        if (saved === 'flow' || saved === 'jump') res = { chatgptTimelineScrollMode: saved };
      }

      const m = res?.chatgptTimelineScrollMode;
      if (m === 'flow' || m === 'jump') this.scrollMode = m;
      this.hideContainer = !!res?.chatgptTimelineHideContainer;
      this.applyContainerVisibility();
      this.toggleDraggable(!!res?.chatgptTimelineDraggable);
      this.toggleMarkerLevel(!!res?.chatgptTimelineMarkerLevel);

      // Load position with auto-migration from v1 to v2
      const position = res?.chatgptTimelinePosition;
      if (position) {
        ...
        // v1 format: migrate to v2 (auto-upgrade)
          ...
          (g.chrome?.storage?.sync || g.browser?.storage?.sync)?.set?.({
            chatgptTimelinePosition: migratedPosition,
          });
        }
      }
      ...
      try {
        ...
        if (onChanged) {
          onChanged.addListener((changes: any, area: string) => {
            if (area !== 'sync') return;
            if (changes?.chatgptTimelineScrollMode) {
              const n = changes.chatgptTimelineScrollMode.newValue;
              if (n === 'flow' || n === 'jump') this.scrollMode = n;
            }
            if (changes?.chatgptTimelineHideContainer) {
              this.hideContainer = !!changes.chatgptTimelineHideContainer.newValue;
              this.applyContainerVisibility();
            }
            if (changes?.chatgptTimelineDraggable) {
              this.toggleDraggable(!!changes.chatgptTimelineDraggable.newValue);
            }
            if (changes?.chatgptTimelineMarkerLevel) {
              this.toggleMarkerLevel(!!changes.chatgptTimelineMarkerLevel.newValue);
            }
            if (changes?.chatgptTimelinePosition && !changes.chatgptTimelinePosition.newValue) {
  ...
  private computeConversationId(): string {
    const conversationId = this.extractConversationIdFromUrl();
  ...
  private extractConversationIdFromUrl(): string | null {
    try {
      const path = window.location.pathname || '';
      const match = path.match(/\/c\/([a-zA-Z0-9_-]+)/);
      if (match && match[1]) return match[1];
      const gptMatch = path.match(/\/g\/([a-zA-Z0-9_-]+)/);
  ...
  private getStarsStorageKey(): string | null {
    return this.conversationId ? `chatgptTimelineStars:${this.conversationId}` : null;
  }
  ...
  private getConversationTitle(): string {
    ...
    // Strategy 2: Try to get from ChatGPT header title
    try {
      const headerTitle =
        document.querySelector('[data-testid="conversation-title"]') ||
        document.querySelector('header [data-testid="conversation-title"]') ||
        document.querySelector('header h1');
      const title = getText(headerTitle);
      if (title && title !== 'ChatGPT' && title !== 'New chat' && title !== 'New Chat') {
        return title;
      }
    } catch (error) {
      console.debug('[Timeline] Failed to get title from header:', error);
    }

    // Strategy 3: Try to get from page title
    ...
    // Strategy 4: Try to get from sidebar conversation list
    ...
    try {
      ...
      for (const selector of selectors) {
        ...
        if (element && element.textContent) {
          ...
          if (text && text.length > 0 && text !== 'New chat' && text !== 'New Chat') {
            return text;
          }
        }
      }
    ...
    // Strategy 5: Use first user message as title (fallback)
    ...
    // Strategy 6: Extract from URL if it contains conversation ID
    try {
  ...
  private async findCriticalElements(): Promise<boolean> {
    ...
    let userOverride = '';
    try {
      userOverride = localStorage.getItem('chatgptTimelineUserTurnSelector') || '';
    } catch {}
    const defaultCandidates = [
      // ChatGPT primary selectors
    ...
    if (!firstTurn) {
      ...
      //   b) auto-detected selector suggests legacy Gemini user-query DOM
      ...
      if (!userOverride && matchedSelector) {
        try {
          localStorage.setItem('chatgptTimelineUserTurnSelectorAuto', matchedSelector);
        } catch {}
      }
      ...
      if (userOverride && matchedSelector && matchedSelector !== userOverride) {
        try {
          localStorage.removeItem('chatgptTimelineUserTurnSelector');
        } catch {}
      }
    }
  ...
  private getConfiguredUserTurnSelector(): string {
    try {
      const user = localStorage.getItem('chatgptTimelineUserTurnSelector');
      if (user && typeof user === 'string') return user;
      const auto = localStorage.getItem('chatgptTimelineUserTurnSelectorAuto');
  ...
  private easeInOutQuad(t: number, b: number, c: number, d: number): number {
    ...
    const spring = (() => {
      try {
        return localStorage.getItem('chatgptTimelineSpring') || 'ios';
      } catch {
        return 'ios';
      }
    })();
  ...
  private getFlowDurationMs(): number {
    try {
      const d = parseInt(localStorage.getItem('chatgptTimelineFlowDurationMs') || '650', 10);
  ...
  private savePosition(): void {
    ...
    if (g.chrome?.storage?.sync?.set) {
      g.chrome.storage.sync.set({ chatgptTimelinePosition: position });
    } else if (g.browser?.storage?.sync?.set) {
      g.browser.storage.sync.set({ chatgptTimelinePosition: position });
    }
  }
  ...
  private async reapplyPosition(): Promise<void> {
    ...
    try {
      res = await new Promise((resolve) => {
        if (g.chrome?.storage?.sync?.get) {
          g.chrome.storage.sync.get(['chatgptTimelinePosition'], (items: any) => {
            if (g.chrome.runtime?.lastError) {
              console.error(
                `[Timeline] chrome.storage.get failed: ${g.chrome.runtime.lastError.message}`,
              );
  ...
  private getLevelsStorageKey(): string | null {
    return this.conversationId ? `chatgptTimelineLevels:${this.conversationId}` : null;
  }
  ...
  private getCollapsedStorageKey(): string | null {
    return this.conversationId ? `chatgptTimelineCollapsed:${this.conversationId}` : null;
  }
