...
function extractThoughtItem(thoughtElement: Element): { header: string; content: string } | null {
  try {
    ...
    console.error('[ChatGPT Voyager] Error extracting thought item:', error);
    return null;
  }
}
  ...
  try {
    ...
    chipElements.forEach((chipEl) => {
      try {
        ...
        console.error('[ChatGPT Voyager] Error extracting browse chip:', error);
      }
    });

    return chips;
  } catch (error) {
    console.error('[ChatGPT Voyager] Error extracting browse chips:', error);
    return [];
  }
}
...
function extractThinkingSection(panelElement: Element): ThinkingSection | null {
  try {
    ...
    console.error('[ChatGPT Voyager] Error extracting thinking section:', error);
    return null;
  }
}
...
export function extractThinkingPanels(): ThinkingContent | null {
  try {
    ...
    if (!deepResearchPanel) {
      console.log('[ChatGPT Voyager] Not a Deep Research conversation');
      return null;
    }
    ...
    if (sections.length === 0) {
      console.log('[ChatGPT Voyager] No thinking content found');
      return null;
    }
    ...
    console.error('[ChatGPT Voyager] Error extracting thinking panels:', error);
    return null;
  }
}
...
function getConversationTitle(): string {
  try {
    ...
    console.error('[ChatGPT Voyager] Error getting conversation title:', error);
  }
