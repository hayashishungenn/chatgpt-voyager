...
export default function Popup() {
  ...
  const handleFormulaCopyFormatChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error('[ChatGPT Voyager] Failed to save formula copy format:', err);
    }
  }, []);
  ...
  const apply = useCallback(
    (settings: SettingsUpdate) => {
      ...
      if (settings.mode) payload.chatgptTimelineScrollMode = settings.mode;
      if (typeof settings.hideContainer === 'boolean')
        payload.chatgptTimelineHideContainer = settings.hideContainer;
      if (typeof settings.draggableTimeline === 'boolean')
        payload.chatgptTimelineDraggable = settings.draggableTimeline;
      if (typeof settings.markerLevelEnabled === 'boolean')
        payload.chatgptTimelineMarkerLevel = settings.markerLevelEnabled;
      if (typeof settings.folderEnabled === 'boolean')
        payload.chatgptFolderEnabled = settings.folderEnabled;
      if (typeof settings.hideArchivedConversations === 'boolean')
        payload.chatgptFolderHideArchivedConversations = settings.hideArchivedConversations;
      if (settings.resetPosition) payload.chatgptTimelinePosition = null;
      ...
      if (typeof settings.watermarkRemoverEnabled === 'boolean')
        payload.chatgptWatermarkRemoverEnabled = settings.watermarkRemoverEnabled;
  ...
  const chatWidthAdjuster = useWidthAdjuster({
    ...
    onApply: useCallback((widthPercent: number) => {
      ...
      try {
        chrome.storage?.sync?.set({ chatgptChatWidth: normalized });
      } catch {}
    }, []),
  });
  ...
  const editInputWidthAdjuster = useWidthAdjuster({
    storageKey: 'chatgptEditInputWidth',
    ...
    onApply: useCallback((widthPercent: number) => {
      ...
      try {
        chrome.storage?.sync?.set({ chatgptEditInputWidth: normalized });
      } catch {}
    }, []),
  });
  ...
  const sidebarWidthAdjuster = useWidthAdjuster({
    storageKey: 'chatgptSidebarWidth',
    ...
    onApply: useCallback((widthPx: number) => {
      ...
      try {
        chrome.storage?.sync?.set({ chatgptSidebarWidth: clamped });
      } catch {}
    }, []),
  });
  ...
  useEffect(() => {
    try {
      ...
      console.error('[ChatGPT Voyager] Failed to get extension version:', err);
    }
  }, []);
      ...
      try {
        ...
        if (!latest) {
          const resp = await fetch(
            'https://api.github.com/repos/hayashishungenn/chatgpt-voyager/releases/latest',
            {
        ...
        if (!cancelled) {
          console.warn('[ChatGPT Voyager] Failed to check latest version:', error);
        }
      }
    };
  ...
  useEffect(() => {
    try {
      chrome.storage?.sync?.get(
        ...
        (res) => {
          const m = res?.chatgptTimelineScrollMode as ScrollMode;
          ...
          if (format === 'latex' || format === 'unicodemath' || format === 'no-dollar')
            setFormulaCopyFormat(format);
          setHideContainer(!!res?.chatgptTimelineHideContainer);
          setDraggableTimeline(!!res?.chatgptTimelineDraggable);
          setMarkerLevelEnabled(!!res?.chatgptTimelineMarkerLevel);
          setFolderEnabled(res?.chatgptFolderEnabled !== false);
          setHideArchivedConversations(!!res?.chatgptFolderHideArchivedConversations);
          ...
          setWatermarkRemoverEnabled(res?.chatgptWatermarkRemoverEnabled !== false);
  ...
  const requestCustomWebsitePermission = useCallback(
    async (domain: string): Promise<boolean> => {
      ...
      try {
        ...
        console.error('[ChatGPT Voyager] Failed to request permissions for custom website:', err);
        setWebsiteError(t('permissionRequestFailed'));
        return false;
      }
    },
  ...
  const revokeCustomWebsitePermission = useCallback(
    async (domain: string) => {
      ...
      try {
        ...
        console.warn('[ChatGPT Voyager] Failed to revoke permission for', domain, err);
      }
    },
  ...
  const latestReleaseUrl = latestReleaseTag
    ? `https://github.com/hayashishungenn/chatgpt-voyager/releases/tag/${latestReleaseTag}`
    : 'https://github.com/hayashishungenn/chatgpt-voyager/releases/latest';
  const currentReleaseTag = toReleaseTag(extVersion);
  const releaseUrl = extVersion
    ? `https://github.com/hayashishungenn/chatgpt-voyager/releases/tag/${currentReleaseTag ?? `v${extVersion}`}`
    : 'https://github.com/hayashishungenn/chatgpt-voyager/releases';
  ...
  return (
    <div className="bg-background text-foreground w-[360px]">
        ...
        <Card className="p-4 transition-shadow hover:shadow-lg">
          ...
          <CardContent className="space-y-3 p-0">
            ...
            <div>
              ...
              {/* ChatGPT Only Notice - moved here since it's about Prompt Manager */}
      ...
      <div className="from-secondary/30 via-accent/10 border-border/50 flex flex-col gap-3 border-t bg-linear-to-br to-transparent px-5 py-4 backdrop-blur-sm">
        ...
        <a
          href="https://github.com/hayashishungenn/chatgpt-voyager"
          target="_blank"
          rel="noreferrer"
